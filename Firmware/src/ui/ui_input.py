...
* `input_evt.e_type`: The event type.
    One of EV_ROTATE for rotation, or EV_BUTTON for button presses
* `input_evt.e_val`: For EV_ROTATE type, this will be either DIR_CW or DIR_CCW.
    For EV_BUTTON, this will be one of SHORT_PRESS or LONG_PRESS
...
async def rotationMonitor():
    ...
    When a rotation event is detected, we will set `rotate_evt` and also the
...
def rotateISR(direction: DIR_CW | DIR_CCW):
    ...
    rotation and encoder buttons into an asyncio app via the Event()
    ...
    """
    # TODO: logging.debug("Encoder rotation callback. dir: %s", direction)
...
def setupEncoder(
    ...
    Note this code assumes a KY-040 style encoder with 'clk' and `data` pins.
    Other encoders could have A/B pins or similar.
    ...
    Args:
        ...
        sw_pin: A `machine.Pin` instance for the pin the `sw` button input is
            ...
            that a press withh take the input low.
